...
map home[9][13];
object *temp[9][13];
void save_c(int round,int jack,int *cards,int *temp_for_sherlock,int turn,int type_of_map,int computer_role,int *innocents){
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 13; j++)
            temp[i][j] = home[i][j].objects;
    }
    FILE *saves=fopen("save_c.bin","ab");
    if(saves==NULL){
        printf("Sorry we can not open the save file.\n");
        return;
    }
    fwrite(&computer_role,sizeof(int),1,saves);
    ...
    for(int i=0;i<8;i++)
        fwrite(&innocents[i],sizeof(int),1,saves);
    int x=0,counter=1;
    for(int i=0;i<8;i++)
